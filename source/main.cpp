...
int main(const int argc, const char *const argv[])
{
    ...
    const auto complTimeDiff = [&]() {
        ...
        for(auto &compNode : computeNodes) {
            maxComplTime = std::max(maxComplTime, compNode.getStatistics().mpi.reduce.lastEnd_tick);
            minComplTime = std::min(minComplTime, compNode.getStatistics().mpi.reduce.lastEnd_tick);
        }
