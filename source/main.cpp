...
int main(const int argc, const char *const argv[])
{
    ...
    const auto timingCost = [&]() {
        ...
        for(auto &compNode : computeNodes) {
            maxDuration = std::max(maxDuration, compNode.getStatistics().mpi.reduceAll.lastDuration());
        }
    ...
    const auto complTimeDiff = [&]() {
        ...
        for(auto &compNode : computeNodes) {
            maxComplTime = std::max(maxComplTime, compNode.getStatistics().mpi.reduceAll.lastEnd_tick);
            minComplTime = std::min(minComplTime, compNode.getStatistics().mpi.reduceAll.lastEnd_tick);
        }
