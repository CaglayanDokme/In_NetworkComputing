...
int main(const int argc, const char *const argv[])
{
    ...
    const auto timingCost = [&]() {
        ...
        for(auto &compNode : computeNodes) {
            maxDuration = std::max(maxDuration, compNode.getStatistics().mpi.allGather.lastDuration());
        }
    ...
    const auto complTimeDiff = [&]() {
        ...
        for(auto &compNode : computeNodes) {
            maxComplTime = std::max(maxComplTime, compNode.getStatistics().mpi.allGather.lastEnd_tick);
            minComplTime = std::min(minComplTime, compNode.getStatistics().mpi.allGather.lastEnd_tick);
        }
