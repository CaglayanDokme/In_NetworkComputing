...
: ISwitch(nextID++, portAmount), firstCompNodeIdx(m_ID * getDownPortAmount())
{
    ...
    m_processorTask = std::thread(&Edge::processorTask, this);
}
...
{
    if(bStopRequested) {
        spdlog::critical("Edge Switch: Tick advance requested after stopping!");
...
bool Edge::isTickProcessedByAll()
{
    return (tickProcessCounter == nextID);
}

void Edge::waitTickCompletion()
{
...
void Edge::processorTask()
{
    m_bRunning = true;
    spdlog::debug("Edge Switch({}): Processor task has been started!", m_ID);
    ...
    if(!bStopRequested) {
        spdlog::error("Edge Switch({}): Processor task has been stopped without being requested!", m_ID);

        throw std::runtime_error("Processor task has been stopped without being requested!");
    }

    spdlog::debug("Edge Switch({}): Processor task has been stopped!", m_ID);
    m_bRunning = false;
}
...
void Edge::process(const size_t sourcePortIdx, std::unique_ptr<Messages::BarrierRequest> msg)
{
    ...
    // Save into flags
    {
        ...
        if(m_barrierRequestFlags.at(downPortIdx)) {
            spdlog::critical("Edge Switch({}): Port #{} already sent a barrier request!", m_ID, sourcePortIdx);

            throw std::runtime_error("Edge Switch: Port already sent a barrier request!");
        }
        else {
...
void Edge::process(const size_t sourcePortIdx, [[maybe_unused]] std::unique_ptr<Messages::BarrierRelease> msg)
{
    ...
    if(sourcePortIdx >= getUpPortAmount()) { // Coming from a down-port
        spdlog::critical("Edge Switch({}): Barrier release received from a down-port({})!", m_ID, sourcePortIdx - getUpPortAmount());
